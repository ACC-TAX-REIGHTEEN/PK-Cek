...
def main():
    ...
    missing_files = [f for f in required_dapur_files if not os.path.exists(os.path.join(dapur_dir, f))]
    
    if missing_files:
        print(f"--> Error: File wajib berikut tidak ditemukan di dalam folder 'Dapur': {missing_files}")
        return
    ...
    try:
        ...
        print("--> [Step 1/5] File 'PK.xls' berhasil disalin ke folder 'Dapur'.")

        print("--> [Step 2/5] Menjalankan 1_Cleaner.py di folder Dapur...")
        subprocess.run([sys.executable, "1_Cleaner.py"], cwd=dapur_dir, check=True)
        print("--> 1_Cleaner.py selesai dijalankan.\n")

        print("--> [Step 3/5] Menjalankan 2_Processing.py di folder Dapur...")
        subprocess.run([sys.executable, "2_Processing.py"], cwd=dapur_dir, check=True)
        ...
        if os.path.exists(dapur_output):
            ...
            print("--> [Step 4/5] File 'PK_Processed.xlsx' berhasil dipindahkan ke folder utama.")
        else:
            print("--> Peringatan: File 'PK_Processed.xlsx' tidak ditemukan di folder Dapur.")

    except subprocess.CalledProcessError as e:
        print(f"\n--> Terjadi kesalahan saat mengeksekusi skrip Python di Dapur: {e}")
        return
    except Exception as e:
        print(f"\n--> Terjadi kesalahan tidak terduga: {e}")
        return

    print("--> [Step 5/5] Membersihkan sisa file Excel dari folder 'Dapur'...")
    cleaned_count = 0
    ...
    print(f"\n--> Selesai! Seluruh alur kerja berhasil dijalankan ({cleaned_count} file sementara dibersihkan).")
